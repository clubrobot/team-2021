...
class PositionDetector:
    ...
    def __init__(self,widthMarker,distanceToMarker):
        ...
        self.KNOW_DISTANCE_TO_MARKER = distanceToMarker
        self.KNOW_WIDTH_MARKER = widthMarker
    ...
    def createProjectionMatrix(self,fov, aspect, near, far):
        f=open("cameraValues.txt","r")
        l=f.read()
        tab=l.split()
        M = np.identity(4)
        M[3, 3] = 1
        M[1, 1] =float(tab[1]) #624.03028306
        M[0, 0] =float(tab[0]) #636.37585975
        M[2, 2] = 1
        M[2, 0] =float(tab[2]) #267.99046485
        M[2, 1] =float(tab[3]) #252.54205713
        return M

    def createProjectionMatrixOld(self,fov, aspect, near, far):
        M = np.identity(4)
        M[3, 3] = 0;
    ...
    def getRay(self,screenPos):
        ...
        clipCoord = [screenPos[0]-self.WIDTH/2, screenPos[1]-self.HEIGTH/2, -1, 1]
    ...
    def update(self):
        ...
        for id in self.markerIds:
            ...
            if (marker > 0):
                ...
                #rotation detection the point (0,0) is a the tx,ty,tz position the point2 is at tx+sqrt(2)Rcos(pi/4+a) , ty , tz+ Rqrt(2)sin(pi/4+a)sin a
                ...
                dx=pos2[0]-pos[0]#=Rcos a
                dz =pos2[2]-pos[2]#=Rsin a
                a=math.atan(dz/dx)
                ...
                #cv2.putText(img, " screen=" + str(point), (10, 400), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.75, (255, 255, 0), 2, cv2.LINE_AA)
                #cv2.putText(img, " ray=" + str(ray), (10, 425), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.75, (255, 255, 0),
                            #2, cv2.LINE_AA)
                ...
                #print('Vitesse=' + str(self.markerSpeed[self.markerIds.index(id)]))
                #print('Rotation'+str(math.degrees(a)))
                print(str(dx/self.KNOW_WIDTH_MARKER)+" "+str(dz/self.KNOW_WIDTH_MARKER)+" "+str(math.degrees(a)))
                print()
