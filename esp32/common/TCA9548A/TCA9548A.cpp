#include "TCA9548A.h"

TCA9548A::TCA9548A() : address_(TCA9548A_DEFAULT_ADDRESS), reset_pin_(-1), wire_(nullptr) {
}

void TCA9548A::begin(uint8_t reset_pin, uint8_t address, TwoWire *wire) {
  this->wire_ = wire;
  this->reset_pin_ = reset_pin;
  this->address_ = address;
  this->reset();
}

uint8_t TCA9548A::get_selected_channels() const {
  this->wire_->requestFrom(this->address_, 1u);
  return this->wire_->read();
}

void TCA9548A::set_selected_channels(uint8_t channels) {
  this->wire_->beginTransmission(this->address_);
  this->wire_->write(channels);
  this->wire_->endTransmission();
}

TwoWire *TCA9548A::get_wire() {
  return this->wire_;
}

void TCA9548A::reset() {
  digitalWrite(this->reset_pin_, LOW);
  delayMicroseconds(1);
  digitalWrite(this->reset_pin_, HIGH);
}
